...
public class PhysicsEngineImpl implements PhysicsEngine {
    ...
    private boolean isPlayerCollidingLeftWall() {
        return (this.gamefield.getPlayer().getPosition().getX().intValue() > this.fieldBounds.getMaxX()
                - X_LEFT_BORDER);
    }
    ...
    private boolean isPlayerCollidingRightWall() {
        return (this.gamefield.getPlayer().getPosition().getX().intValue() < this.fieldBounds.getMinX()
                - X_RIGHT_BORDER);
    }
